...
- [ ] 梳理 system/developer/user/assistant/tool 消息的职责和信任边界。
- [ ] 掌握 JSON Schema、结构化输出校验、Tool Call ID 和工具结果回传。
- [ ] 理解流式响应、超时、取消、速率限制和非确定性输出。
- [ ] 阅读 [ReAct](https://arxiv.org/abs/2210.03629) 的方法与实验，提炼“推理—行动—观察”循环。
- [ ] 明确最大步数、Token/费用预算、超时和显式完成条件。

### 编码实验

- [ ] 实现支持结构化输出和流式响应的模型调用适配层。
- [ ] 实现 `model -> tool -> observation -> model` 最小循环。
- [ ] 为每次模型调用和工具调用生成 Trace ID、开始时间、结束时间和结果状态。
- [ ] 加入未知工具、参数校验失败、工具异常、超时和模型拒答处理。
- [ ] 编写至少 8 个测试，覆盖正常完成、最大步数、错误恢复和提前终止。

### 面试与验收

- [ ] 写出 Agent 与 Workflow、Chatbot、RAG Pipeline 的边界。
- [ ] 回答至少 5 道 Agent Loop 设计题。
- [ ] 完成 10 分钟演示，并能逐步解释一条完整轨迹。
- [ ] 核心循环保持简单、可以独立运行，测试全部通过。
